...
use crate::foundation_analyzer::{FoundationAnalyzer, FoundationQuery};
use crate::models::{
...
use crate::structure_comparator::StructureComparator;
use crate::vr_siege_tower::VrSiegeTower;
...
pub type MoatQuery = FoundationQuery;
...
pub async fn compare_dynasty(
    ...
    let comparison = comparator.compare_ancient_dynasties(wind_speed, tilt_deg);
...
pub async fn compare_cross_era(
    ...
    let comparison = comparator.compare_cross_era(tilt_deg);
    (StatusCode::OK, Json(ApiResponse::success(comparison)))
}
...
pub async fn moat_analysis(
    ...
    let analyzer = FoundationAnalyzer::new();
    let query = FoundationQuery {
        moat_distance: params.moat_distance,
        moat_depth: params.moat_depth,
        water_table_depth: params.water_table_depth,
        wind_speed: params.wind_speed,
        tilt_deg: params.tilt_deg,
        soil_type: params.soil_type,
    };
    let result = analyzer.analyze_moat_by_tower_id(tower_id, query);
...
pub async fn climbing_viewpoints(
    ...
    let experience = vr.build_experience(tower_id);
